...
namespace Step15
{
  ...
  private:
    ...
    void output_results(const unsigned int refinement) const;
  ...
  { 
    ...
    auto worker = [&](const decltype(dof_handler.begin_active()) &cell,
     ...
     auto copier = [&](const MeshWorker::CopyData<1, 1, 1> &copy_data) {
        ...
        copy_data.matrices[0], copy_data.vectors[0], copy_data.local_dof_indices[0], system_matrix, system_rhs);
  ...
  template <int dim>
  void
  MinimalSurfaceProblem<dim>::output_results(const unsigned int refinement) const
  {     
  ...
  void MinimalSurfaceProblem<dim>::run()
  {
    ...
    while (first_step || (previous_res > 5e-2))
      {
        if (first_step == true)
          {
